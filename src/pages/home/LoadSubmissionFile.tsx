...
import { parseProsperitySubmissionFile } from '../../utils/algorithm.tsx';
...
export function LoadSubmissionFile(): ReactNode {
    ...
    (files: File[]) =>
      new Promise<void>((resolve, reject) => {
        ...
        reader.addEventListener('load', () => {
          try {
            setAlgorithm(parseProsperitySubmissionFile(reader.result as string));
            navigate('/visualizer');
            resolve();
          } catch (err: any) {
            reject(err);
  ...
  const onReject = useCallback((rejections: FileRejection[]) => {
    ...
    for (const rejection of rejections) {
      const errorType = {
        'file-invalid-type': 'Invalid type, only submission JSON/log files are supported.',
        'file-too-large': 'File too large.',
        'file-too-small': 'File too small.',
        'too-many-files': 'Too many files.',
      }[rejection.errors[0].code]!;

      messages.push(`Could not load submission from ${rejection.file.name}: ${errorType}`);
    }
  ...
  return (
    <HomeCard title="Load Prosperity submission file">
      <Text>
        Supports the JSON submission/result files downloaded from Prosperity that contain top-level
        <code> activitiesLog </code>
        and
        <code> logs </code>
        fields. Use this loader for files like <code>216605.log</code>.
      </Text>
